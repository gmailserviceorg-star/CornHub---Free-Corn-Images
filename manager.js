...
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>RAC Terminal</title>
...
async function run(command){
  ...
  if(m){try{const r=await fetch('/api/run-device-app',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({app:m[1]})});const d=await r.json();print(d.message||d.error||'Command failed.');}catch(e){print('Unable to reach RAC controller.')}return}
...
const server=http.createServer(async (req,res)=>{
  try {
    ...
    if(req.url==='/api/run-device-app' && req.method==='POST'){
      ...
      if(DEVICE_CODE==='PLACEHOLDER-CODE') return json(res,400,{error:'Set DEVICE_CODE in code.js first.'});
      if(!allowed.includes(b.app)) return json(res,400,{error:'App is not in the RAC allowlist.'});
...
server.listen(PORT,'127.0.0.1',()=>console.log('RAC manager listening at http://localhost:'+PORT));
